...
import { Character, Comic } from '../../../store/actions/actionsTypes/ActionsTypes';
...
const CharacterDetail: React.FC<CharacterDetailProps> = ({ getSingleCharacter, singleCharacter, getSelectedComic, relatedComics, navigation }) => {
  const styles = characterDetailStyles

  const [characterState, setCharacterState] = useState(true)

  useEffect(() => {
    navigation.state.params.length > 0 && getSingleCharacter(navigation.state.params)
  }, [getSingleCharacter])
        ...
        <LinearGradient
          ...
          <ScrollView>
            {singleCharacter.thumbnail.extension === 'jpg' ?
              <Image
                source={{ uri: `${singleCharacter.thumbnail.path}/portrait_uncanny.jpg` }}
            ...
            <FlatList
              ...
              numColumns={2}
              style={{ alignSelf: 'center', marginBottom: hp('24%') }}
              bounces={false}
              renderItem={({ item }) =>
...
const mapDispatchToProps = dispatch => ({
  getSingleCharacter: (characterName) => dispatch(getSingleCharacterAction(characterName)),
  getSelectedComic: (comicId: number, comics: Comic[], characterState: boolean) => dispatch(getComicByIdAction(comicId, comics, characterState)),
});
